                ...
                @foreach ($productos as $product)
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="confirmDeleteModalLabel">Confirmar
                                                {{ $product->visible ? 'ocultadita' : 'mostradita' }}</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
                                            ¿Estás seguro de que deseas {{ $product->visible ? 'ocultar' : 'mostrar' }} el
                                            producto
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form method="POST" action="{{ route('productos.hide', $product->id) }}"
                                                ...
                                                @csrf
                                                <button type="submit"
                                                    class="btn btn-danger">{{ $product->visible ? 'Ocultar' : 'Mostrar' }}</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <tr hidden id="edit{{ $product->id }}">
                        ...
                        <form action="{{ route('productos.update', $product->id) }}" class="form" method="POST">
                            ...
                            <td>{{ $product->visible ? 'Visible' : 'oculto' }}</td>
                            <td><input type="text" name="name" value="{{ $product->name }}" class="form-control">
                            </td>
                                ...
                                <select name="coupon_id" id="coupon_id" class="form-select">
                                    ...
                                    @foreach ($coupons as $coupon)
                                        <option value="{{ $coupon->id }}"
                                            @if ($coupon->id == $product->coupon_id) selected @endif>
                                            {{ $coupon->name . ' ' . $coupon->discount . '%' }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td><input type="number" name="stock" value="{{ $product->stock }}" class="form-control">
                            </td>
                            <td><input type="text" name="specs" value="{{ $product->specs }}" class="form-control">
                            </td>
                            <td><input type="text" name="features" value="{{ $product->features }}"
                                    class="form-control">
                            </td>
                            <td>
                            ...
                            <td><input type="text" name="color" value="{{ $product->color }}" class="form-control">
                            </td>
